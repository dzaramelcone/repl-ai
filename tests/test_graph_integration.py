...
async def test_graph_code_then_complete():
    ...
    llm = MockLLM(
        [
            ...
            "<reflection><is_complete>true</is_complete><reasoning>Variable set and printed</reasoning></reflection>",
        ]
    )
    graph = build_agent_graph(llm=llm, max_turns=5)
...
async def test_graph_multi_turn():
    ...
    llm = MockLLM(
        [
            ...
            "<reflection><is_complete>false</is_complete><reasoning>Need to also print</reasoning></reflection>",
            "<assistant-chat>Now print:</assistant-chat><assistant-repl-in>print(x)</assistant-repl-in>",
            "<reflection><is_complete>true</is_complete><reasoning>Done</reasoning></reflection>",
        ]
    )
    graph = build_agent_graph(llm=llm, max_turns=5)
...
async def test_graph_max_turns_limit():
    ...
    llm = MockLLM(
        [
            ...
            "<reflection><is_complete>false</is_complete><reasoning>Never done</reasoning></reflection>",
        ]
    )
